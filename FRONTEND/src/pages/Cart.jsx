...
const Cart = () => {
  ...
  if (!user) {
    return (
      <div className="bg-[#1a0f05] min-h-[80vh] flex items-center justify-center pt-20 px-4">
        <div className="bg-[#2a1b0a] border border-[#5C3D11]/50 p-10 rounded-2xl text-center max-w-md w-full shadow-2xl">
          <div className="w-20 h-20 bg-[#1a0f05] rounded-full flex items-center justify-center mx-auto mb-6 border border-[#5C3D11]">
  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="bg-[#1a0f05] min-h-[80vh] flex flex-col items-center justify-center pt-20 px-4">
        <div className="w-32 h-32 bg-[#2a1b0a] rounded-full flex items-center justify-center border border-[#5C3D11]/50 shadow-inner mb-8 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
          <ShoppingBag className="h-16 w-16 text-orange-500/50" />
        </div>
        <h2 className="text-4xl font-extrabold text-white mb-4 text-center">Tu carrito está vacío</h2>
  ...
  return (
    <div className="bg-[#1a0f05] min-h-screen pt-28 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Row Cabecera Título */}
        <div className="flex items-center space-x-4 mb-10 border-b border-[#5C3D11]/30 pb-6">
            <div className="bg-orange-500 p-3 rounded-lg shadow-lg shadow-orange-500/20">
                <ShoppingBag className="text-white h-6 w-6" />
            </div>
            <div>
                <h1 className="text-3xl lg:text-4xl font-bold text-white tracking-tight">Carrito de Compras</h1>
                <p className="text-orange-400 mt-1 font-medium">{cart.items.reduce((acc, item) => acc + item.quantity, 0)} artículo(s) seleccionados</p>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#2a1b0a]/50 text-xs font-bold text-gray-500 uppercase tracking-wider px-6 py-3 rounded-t-xl hidden sm:grid sm:grid-cols-12 gap-4 border-b border-[#5C3D11]/30">
            ...
            {cart.items.map((item) => (
              <div 
                key={item.id} 
                className="bg-[#2a1b0a] border border-[#5C3D11]/50 rounded-xl p-4 sm:p-6 shadow-md hover:border-orange-500/30 transition-colors flex flex-col sm:grid sm:grid-cols-12 gap-6 items-center"
              >
                {/* 1. Descripción visual del Item (Izquierda col 1 a 6) */}
                <div className="col-span-6 flex items-center w-full">
                  <div className="w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0 bg-[#1a0f05] rounded-lg border border-[#5C3D11]/30 p-2 overflow-hidden flex items-center justify-center relative">
                    {/* Badge Sale si el Contexto inyecta producto en promo */}
                    {item.product.is_on_sale && item.product.discount_percentage > 0 && (
                        <div className="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-bl-lg z-10">
                  ...
                  <div className="ml-4 flex-1">
                    <Link to={`/products/${item.product.id}`} className="block text-lg font-bold text-white hover:text-orange-400 transition-colors mb-1">
                        {item.product.name}
                    </Link>
                    
                    {/* Subprecio por unidad aclaratorio */}
                    <div className="flex items-center space-x-2 mt-1">
                        <span className="text-orange-400 font-semibold">{formatPrice(item.product.sale_price || item.product.price)}</span>
                        {(item.product.is_on_sale && item.product.discount_percentage > 0) && (
                ...
                <div className="col-span-3 flex justify-center w-full sm:w-auto border-t border-b border-[#5C3D11]/30 sm:border-0 py-4 sm:py-0">
                  <div className="flex items-center bg-[#1a0f05] rounded-xl border border-[#5C3D11] p-1 shadow-inner">
                    <button
                      onClick={() => updateItemQuantity(item.id, item.quantity - 1)}
                      disabled={item.quantity <= 1} // No bajar de 1, es preferible "eliminar" para ir a 0
                      className="p-2 text-gray-400 hover:text-white hover:bg-[#2a1b0a] rounded-lg disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                    >
                      <Minus className="h-4 w-4" />
                    </button>
                    <span className="w-12 text-center text-white font-bold text-lg select-none">
                        {item.quantity}
                    </span>
                    <button
                      onClick={() => updateItemQuantity(item.id, item.quantity + 1)}
            ...
            <div className="pt-4">
                <Link to="/products" className="inline-flex items-center text-gray-400 hover:text-orange-400 font-medium transition-colors">
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#2a1b0a] border border-[#5C3D11]/50 rounded-2xl p-6 shadow-xl sticky top-32">
              ...
              <div className="border-t border-[#5C3D11]/50 pt-6 mb-8">
                <div className="flex justify-between items-end">
                  <span className="text-lg font-bold text-white">Total Final</span>
                  <div className="text-right">
                      <span className="text-3xl font-extrabold text-orange-500 block leading-none">{formatPrice(cart.total)}</span>
              ...
              <Button 
                ...
                className="w-full shadow-orange-500/25 flex items-center justify-center"
              >
              ...
              <div className="mt-6 flex justify-center space-x-2">
                  ...
                  <div className="w-10 h-6 bg-white/10 rounded flex items-center justify-center text-[8px] font-bold text-white uppercase opacity-50">VISA</div>
                  <div className="w-10 h-6 bg-white/10 rounded flex items-center justify-center text-[8px] font-bold text-white uppercase opacity-50">MC</div>
              </div>
            </div>
          </div>
        </div>

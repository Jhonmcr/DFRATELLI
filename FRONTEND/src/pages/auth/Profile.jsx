...
const Profile = () => {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            ...
            case 'SHIPPED':   return "bg-amber-500/20 text-amber-600 border border-amber-500/50";
            ...
            default:          return "bg-gray-500/20 text-gray-600 border border-gray-500/50";
        }
    };
    ...
    const renderProfileTab = () => (
        <div className="bg-white border border-amber-200 rounded-2xl p-8 shadow-xl">
            <h3 className="text-2xl font-bold text-gray-900 mb-6 border-b border-amber-200 pb-4">Detalles de la Cuenta</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6 text-gray-700">
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Correo Electrónico</p>
                        <p className="font-medium text-gray-900 text-lg bg-amber-50 px-4 py-3 rounded-lg border border-amber-300">{fullProfile?.email}</p>
                    </div>
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Nombre de Usuario</p>
                        <p className="font-medium text-gray-900 text-lg bg-amber-50 px-4 py-3 rounded-lg border border-amber-300">{fullProfile?.username || user?.username}</p>
                    </div>
                </div>
                ...
                <div className="space-y-6 text-gray-700">
                    <div>
                        ...
                        <div className="bg-amber-50 px-4 py-3 rounded-lg border border-amber-300 flex items-center justify-between">
                            <span className="font-medium text-gray-900 text-lg capitalize">{fullProfile?.role?.toLowerCase() || 'Cliente'}</span>
                            <ShieldCheck className={`h-5 w-5 ${(fullProfile?.role === 'ADMIN' || fullProfile?.role === 'SUPERADMIN') ? 'text-amber-500' : 'text-gray-500'}`} />
                        </div>
                    </div>
                    ...
                    {(fullProfile?.role === 'ADMIN' || fullProfile?.role === 'SUPERADMIN') && fullProfile?.unique_code && (
                        <div>
                            ...
                            <p className="font-mono text-amber-600 bg-orange-900/10 px-4 py-3 rounded-lg border border-amber-500/30 font-bold uppercase tracking-[0.2em]">
            ...
            <div className="mt-8 pt-8 border-t border-amber-200">
                <p className="text-gray-600 text-sm mb-4">La modificación de perfil y contraseñas está sujeta a verificación de seguridad bidireccional.</p>
                <div className="flex gap-4">
                    <button disabled className="px-6 py-2.5 bg-amber-50 text-gray-500 border border-amber-200 rounded-lg cursor-not-allowed">Editar Información</button>
                    <button disabled className="px-6 py-2.5 bg-amber-50 text-gray-500 border border-amber-200 rounded-lg cursor-not-allowed">Cambiar Contraseña</button>
                </div>
            </div>
        </div>
    );
    ...
    const renderOrdersTab = () => (
        <div className="space-y-6">
            <h3 className="text-2xl font-bold text-gray-900 mb-6">Mis Compras</h3>
            {orders.length === 0 ? (
                <div className="bg-white border border-amber-200 rounded-2xl p-12 text-center shadow-inner">
                    <Package className="h-16 w-16 text-gray-600 mx-auto mb-4" />
                    <p className="text-xl text-gray-900 font-medium mb-2">Aún no tienes historial de compras</p>
                    <p className="text-gray-600">Cuando realices compras aparecerán aquí listadas y documentadas.</p>
                </div>
            ) : (
                                ...
                                {order.items.slice(0, 2).map(item => (
                                    <div key={item.id} className="flex justify-between items-center text-sm">
                                        <div className="text-gray-700 flex items-center">
                                            <span className="bg-amber-50 text-gray-600 px-2 py-0.5 rounded mr-3 border border-amber-200">x{item.quantity}</span>
                                            {item.product?.name || 'Recurso Histórico Eliminado'}
                                        </div>
                                    </div>
                                ))}
                                {order.items.length > 2 && (
                                    <p className="text-xs text-amber-500 font-medium cursor-pointer">+ {order.items.length - 2} items adicionales... (ver detalle completo)</p>
                                )}
                            </div>

                            <div className="flex justify-between items-end pt-4 bg-amber-50 -mx-5 -mb-5 px-5 py-4 rounded-b-xl border-t border-amber-300">
                                <button className="text-sm font-medium text-amber-500 hover:text-amber-600 transition-colors flex items-center">
    ...
    return (
        <div className="bg-amber-50 min-h-screen pt-28 pb-16">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white border border-amber-300 rounded-2xl p-6 sm:p-10 mb-8 shadow-xl relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end z-10 relative">
                        <div>
                            <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight mb-2">
                                Hola, <span className="text-amber-500">{fullProfile?.first_name || fullProfile?.username || 'Usuario'}</span>
                            </h1>
                            <p className="text-gray-600">Panel administrativo personal y preferencias de tu cuenta.</p>
                        </div>

                        {/* Ticker / Recuento resumido para clientes (Incentiva recompra) */}
                        <div className="mt-6 md:mt-0 flex gap-4">
                            <div className="bg-amber-50 border border-amber-300 px-5 py-3 rounded-xl min-w-[120px]">
                            ...
                            <div className="bg-amber-50 border border-amber-500/30 px-5 py-3 rounded-xl min-w-[140px]">
                                <span className="block text-xs text-amber-500/70 font-bold uppercase tracking-wider mb-1">Total Histórico</span>
                                <span className="text-2xl font-bold text-amber-500">{formatPrice(stats?.totalSpent || 0)}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-2">
                        ...
                        <button
                            ...
                            className={`w-full flex items-center px-4 py-3 rounded-xl transition-all ${activeTab === 'orders' ? 'bg-amber-500 text-gray-900 shadow-lg shadow-amber-500/20' : 'bg-white text-gray-600 hover:bg-[#3a2610] hover:text-gray-900 border border-amber-200'}`}
                        >
                        ...
                        <button
                            ...
                            className={`w-full flex items-center px-4 py-3 rounded-xl transition-all ${activeTab === 'notifications' ? 'bg-amber-500 text-gray-900 shadow-lg shadow-amber-500/20' : 'bg-white text-gray-600 hover:bg-[#3a2610] hover:text-gray-900 border border-amber-200'}`}
                        >
                        ...
                        <div className="pt-8 border-t border-amber-200 mt-8">
                            <button
                                onClick={logout}
                                className="w-full flex items-center justify-center px-4 py-3 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-gray-900 transition-all border border-red-500/20 font-medium"
                            >
                    ...
                    <div className="lg:col-span-3">
                        ...
                        {activeTab === 'notifications' && (
                            <div className="bg-white border border-amber-200 rounded-2xl p-12 text-center shadow-inner">
                                ...
                                <p className="text-xl text-gray-900 font-medium mb-2">No hay notificaciones nuevas</p>
                                <p className="text-gray-600">Te avisaremos sobre envíos de tus compras o cambios en cuenta.</p>
                            </div>
                        )}
                    </div>
                </div>
